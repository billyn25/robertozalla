...
/* V87: dedicated mobile sheet renderer — same live controls, no duplicated fields. */
(() => {
  ...
  function rememberAndMove(node, target) {
    ...
    const marker = document.createComment('v87-origin');
  ...
  function restoreDesktop() {
    ...
    shell = null;
    active = false;
    document.documentElement.classList.remove('v87-mobile-sheet-active');
  }
  ...
  function buildMobile() {
    ...
    shell.className = 'v87-mobile-sheet';
    paper.insertBefore(shell, paper.firstChild);

    const top = document.createElement('section');
    top.className = 'v87-card v87-top';
    const work = document.createElement('section');
    work.className = 'v87-work';
    const concepts = document.createElement('section');
    concepts.className = 'v87-concepts';
    const details = document.createElement('section');
    details.className = 'v87-details';
    const totals = document.createElement('section');
    totals.className = 'v87-money';
    const signatures = document.createElement('section');
    signatures.className = 'v87-signatures';

    shell.append(top, work, concepts, details, totals, signatures);

    const paperHeader = paper.querySelector('.paper-header');
    const topDetails = paper.querySelector('.top-details');
    const description = paper.querySelector('.service-description');
    const items = paper.querySelector('.items-section');
    const bottom = paper.querySelector('.bottom-grid');
    const consent = paper.querySelector('.consent-signatures');

    rememberAndMove(paperHeader, top);

    if (topDetails) {
      const client = topDetails.querySelector('.client-box');
      const services = topDetails.querySelector('.service-boxes');
      if (client) rememberAndMove(client, work);
      if (services) rememberAndMove(services, work);
    }
    rememberAndMove(description, work);
    rememberAndMove(items, concepts);

    if (bottom) {
      const left = bottom.querySelector('.bottom-left');
      const totalsPanel = bottom.querySelector('.totals-panel');
      if (left) rememberAndMove(left, details);
      if (totalsPanel) rememberAndMove(totalsPanel, totals);
    }
    rememberAndMove(consent, signatures);

    // Semantic hooks for generated concept rows; MutationObserver keeps new rows consistent.
    const markRows = () => {
      document.querySelectorAll('#itemsBody tr').forEach(row => {
        row.classList.add('v87-concept-row');
        const cells = row.querySelectorAll('td');
        cells.forEach(td => {
          const key = (td.dataset.label || '').toLowerCase();
          if (key) td.classList.add('v87-' + key.replace(/[^a-z0-9]+/g,'-'));
        });
      });
    };
    markRows();
    const body = document.getElementById('itemsBody');
    if (body) new MutationObserver(markRows).observe(body,{childList:true});

    document.documentElement.classList.add('v87-mobile-sheet-active');
    active = true;
  }

  function sync() {
    if (MQ.matches) buildMobile();
    else restoreDesktop();
  }
